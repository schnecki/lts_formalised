...
\begin{definition}
  ...
  \begin{align*}
    \ts(p) \defsym 1 + \ceil[\Bigg]{\frac{ \zs \cdot 2 \sigma}{\frac{1}{p} \cdot \sum_{i=0}^{p}{f(i)}}} +
    \MS[t]{\text{seasonal stuff todo}} \tcom
  \end{align*}
  where $\zs$ can be used to select the confidence interval being considered.
  % This
...
The reduce the impact of outliers we average the actual releases over $ts(p)$ values.
